// UWAGA PROGRAM NIE SPRAWDZA PODANEGO KOSZTU OPTYMALNEGO
// Program sprawdza po kazdym powtorzeniu tylko poprawnosc wyliczonej sciezki z podana
// nazwa_instancji ilość powtórzeń, koszt optymalny, [sciezka]
// Koszt optymalny moze byc dowolny (ale musi byc wpisana jakas liczba)
    ...
    while(getline(file_in, instance_info) ){ 
        ...
        if(instance_name[instance_name.size()-1] == 'v'){               // ostatnia linia z "conf.ini" czyli nazwa pliku csv jest wpisywana na końcu tego pliku
            ...
            while(rep++ <=  reps){
                ...
                solution_cost = hk.calculate(vertices, distances);
                
                auto stop = std::chrono::high_resolution_clock::now();
                hk.~Hk();
                
                auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
                // wpisanie informacji o instancji przed podanie pierwszego czasu
                // nazwa, ilosc powtórzen koszt, znaleziona sciezka
                ...
                if( rep == 2){
                    ...
                    file_out << instance_name + " " + std::to_string(reps)
                        + " " + std::to_string(solution_cost[vertices+1]) + " " + ret_path << std::endl; 
                }
                ...
                for(int i = 0; i <= vertices; i++){                     // sprawdzenie czy ścieżka zgadza się z optymalną dla instancji danej  podaną w pliku conf.ini  
                    if(solution_cost[i] != best_path[i])
                        path = false;
                    if(solution_cost[i] != best_path[vertices-i])
                        path_sym = false;
                }
                ...
                if(path == false){                                      // jeśli algorytm wyznaczył inną scieżkę podajemy ją  
                    ...
                    if(path_sym == true)
                        std::cout << "PATHS ARE SYMMETRICAL" << std::endl;
                    std::cout << "Path: ";
                ...
                if(solution_cost[vertices+1] != best_cost){             // jeśli algorytm wyznaczył inny koszt podajemy go
                    std::cout << "Attention! diffrent cost for instance: " << instance_name << " at repetition: " << rep-1 << std::endl;
